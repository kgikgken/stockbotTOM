...
- エンジンS(ショート・(b)逆流戻り売りのみ自動): 流出セクター(成熟以降)の2〜3日リバウンド
- エンジンB疑い(PEAD): 決算月(2/5/8/11)のみ・上方イベント痕跡+ドリフト限定
- エンジンG: チャット側 / 旧C/E: エンジンAの補助表示に統合

ボット出力は全件「仮点灯(未確認)」。本命昇格はiSPEED照合＋チャット側ゲート0/3後。
単一ソースでも各エンジン上限(A=高/S・B=中)まで確信度到達可(減点なし)。
"""
...
def run_screen(universe: pd.DataFrame, ohlcv: Dict[str, pd.DataFrame],
    ...
    pool: List[Candidate] = []   # 参考層プール(低conf・超過分)
    considered = 0
