...
## 応答言語
- ユーザーへの応答は日本語で書く
- コード・コミットメッセージ・コード内コメントは英語のまま
- 報告で「完了」「準備できている」などの自己評価を書かない。事実と数値のみ
...
- 取得ウィンドウ**全体**が一様に壊れると `check_splits` を素通りする（2026-09-13 に
  1909.T で発生。終値が一定倍率 4,399,472 倍＝時価総額らしき値になり、出来高が全行 0）。
  段差はフレームの中ではなく**新規取得行と既存の古い行の継ぎ目**にある → `upsert` の
  `seam_issues` で継ぎ目を検査し、引っかかった銘柄の新規行は store に入れない。
  **ただし取り直しても直らないことがある** —— 1909.T は全履歴再取得で 400 行の破損が
  全 2,597 行に広がった（yfinance 側が全期間を壊れた値で返すようになっていた）。
  `refetch-tickers` は置換前に `fetch_sanity_issues` で見て、壊れていれば置換しない。
  直らない銘柄は `layer1.DATA_QUALITY_EXCLUDED_2026_09` に入れて計算から外す
  （**store の行は消さない**。証拠を残し、直れば戻せる）。
  **9900.T（T-402）と同じ構造で 2 回目**
