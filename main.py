...
from utils.util import jst_today_str, jst_today_date, jst_now, parse_event_datetime_jst
from utils.market import enhance_market_score
from utils.sector import top_sectors_5d
from utils.scoring import score_stock, calc_inout_for_stock, calc_ma
...
def run_swing(today_date, mkt_score: int) -> List[Dict]:
    ...
    for _, row in uni.iterrows():
        ...
        tp_pct = float(rr_info["tp_pct"])     # 小数(0.066=+6.6%)
        sl_pct = float(rr_info["sl_pct"])     # 小数(-0.027=-2.7%)
...
def run_day(today_date, mkt_score: int) -> List[Dict]:
    ...
    for _, row in uni.iterrows():
        ...
        # デイは「足の並び」「勢い」「出来高」を重視（utils/day.py）
        ...
        # 直近足でRRっぽい目安（簡易）：直近の5m ATR相当でSL、直近高値でTP
