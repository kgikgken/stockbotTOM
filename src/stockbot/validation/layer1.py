...
DATA_QUALITY_EXCLUDED_T402 = frozenset({
    "1364.T", "3477.T", "4316.T", "5103.T", "6731.T",
    "6834.T", "7649.T", "7877.T", "7983.T", "9900.T",
})

# 2026-09-17 追加（設計責任者の指示）。**T-402 の 10 銘柄とは別の集合にしてある** ——
# 上の定数は「適用時の変更禁止」と事前登録されているので、字面を変えない。
# 機構（`exclude_data_quality_tickers` と `DATA_QUALITY_EXCLUDED_TICKERS`）は同じ。
#
# **yfinance が価格欄に時価総額らしき値を返した破損。** 終値が一定倍率で跳ね上がり、
# 出来高が 0 になる。`check_splits` は素通りする（取得ウィンドウ全体が一様に壊れると
# フレーム内に段差が出ない。CLAUDE.md の落とし穴）。
#
# - `1909.T` 日本ドライケミカル: 2026-09-13 に 400 行（2025-01-24〜）が破損。
#   2026-09-17 の全履歴再取得で **2,597 行全部**が破損した（取得元が壊れているので
#   取り直しても直らないことが確定した）
# - `8303.T`: 125 行（2023-09-27〜）が破損。2026-09-17 の走査で発見
#
# **store から行は消さない**（証拠を残す。yfinance 側が直れば除外を外せる）。
# 除外はユニバース構築・ベンチマーク・パターン検出・配信のすべてに効かせる。
# データ品質による除外なので**検定数は増えない**。
DATA_QUALITY_EXCLUDED_2026_09 = frozenset({"1909.T", "8303.T"})

DATA_QUALITY_EXCLUDED_TICKERS = DATA_QUALITY_EXCLUDED_T402 | DATA_QUALITY_EXCLUDED_2026_09
