...
def compute_tp_sl_rr(hist: pd.DataFrame, mkt_score: int, for_day: bool = False) -> dict:
    """Variable RR based on structure + ATR; safe/fixed implementation (no syntax traps)."""
    df = hist.copy()
    close = df["Close"].astype(float)
    ...
    # Entry (pullback basis)
    entry = ma20 - 0.5 * atr
    entry_basis = "pullback"

    # Shallow pullback in strong trend, but never chase above price
    if price > ma5 > ma20:
    ...
    # TP: near 60d high (resistance) with regime tweak
    hi_window = 60 if len(close) >= 60 else len(close)
    high_60 = float(close.tail(hi_window).max())
    cap = 0.22 if not for_day else 0.08
    tp_price = min(high_60 * 0.995, entry * (1.0 + cap))
    ...
    tp_pct = float(np.clip(tp_pct, 0.03, 0.35))
