"""v7データ層 — 実データ取得(粘りループ・締切ガード付き)とDRYRUN合成データ。
...
def _mk_v6(seed: int, n: int, mode: str) -> pd.DataFrame:
    """v7の検証に意味のあるパターンを生成する。

    v7は「点灯するか」ではなく「5次元それぞれが何点か」を測る設計なので、
    合成データも次元スコアが散らばるように作り分ける。特定パターンだけを
    再現すると全銘柄のスコアが同じ値に固まり、Phase 2 のヒストグラム確認
    (§4.2.1 θの妥当性判断)や Level 1 の相関検証(§11.2)が成立しない。

      vcp       … ①高 ④高 ⑤高（収縮の連鎖＋出来高枯渇）
      pullback  … ①高 ④中 ⑤中（浅い押し）
      pivot     … ①高 ④低 ⑤低（ベース明けの伸び）
      choppy    … ①中 ⑤低（方向感なく振れる。次元が割れる例）
      downtrend … ①0（ハードフロア未達の確認用）
    """
    ...
    if mode == "vcp":
        ...
        # 方向感なく振れる。①が中程度、⑤が低くなり、次元スコアが割れる想定。
        # 全銘柄が同じスコアに固まるとヒストグラムも相関検証も意味を失うため、
        # 「どの次元も高くない」銘柄を意図的に混ぜる。
        base = np.linspace(600, 640, n)
        wave = 60 * np.sin(np.linspace(0, 14 * np.pi, n))
        price = base + wave
...
def _synthetic(tickers: List[str]) -> Tuple[Dict[str, pd.DataFrame], dict]:
    """DRYRUN用の合成データ。v7の次元スコアが散らばる配分にする。
