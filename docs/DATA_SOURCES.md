...
### 実測: 前向きのカバー率は季節性が大きい（2026-08-31）

上の「G0ゲートには実用上十分」という評価は、**決算期の谷間では成り立たない。**
2026-08-31 時点の実測:

| 項目 | 値 |
| --- | --- |
| `earnings_schedule.csv` の行数 | 3,213 |
| 日付の範囲 | 2026-07-03 〜 2026-09-17 |
| 判定日（2026-08-31）以降の行 | 225（7.0%） |
| 判定日より過去の行 | 2,988（93.0%） |
| ユニバース通過 1,323 銘柄のうち、判定日以降の予定日が取れたもの | **37（2.8%）** |

取得そのものは成功している（`[references] 決算発表予定日 3213 件を保存`）。JPX が
公開するのは「直近に決算期末を迎えた会社」の予定日だけなので、8 月末のように第1四半期の
発表が終わり次の波（10〜11 月）がまだ公開されていない時期には、ファイルの大半が過去日に
なる。`next_earnings_business_days()` は `date >= asof` で絞るため、これらは全て
「予定日なし」（None）を返す。

**この性質を前提にする条件側の扱い**は `docs/SCREENER.md` §2.6（A4）を参照。
決算日が取れないことを不利に倒さず、`a4_earnings_unknown` を立てて表示で明示する。
